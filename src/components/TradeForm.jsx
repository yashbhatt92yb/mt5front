...
const TradeForm = () => {
  ...
  const [type, setType] = useState('0'); // 0: Buy, 1: Sell, 2: Buy Limit, 3: Sell Limit
  ...
  const isPending = ['2', '3'].includes(type);
    ...
    <div className="rounded-2xl bg-slate-900 p-6 shadow-sm ring-1 ring-slate-800">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className={!isPending ? 'col-span-2' : ''}>
             <div className="flex justify-between items-center text-xs text-slate-500 mb-1 px-1">
                <span>Current Ask: <span className="text-emerald-500">{currentQuote.ask.toFixed(5)}</span></span>
                <span>Current Bid: <span className="text-rose-500">{currentQuote.bid.toFixed(5)}</span></span>
             </div>
          </div>
        </div>
        ...
        <button
          ...
          className={`w-full py-3 rounded-xl font-bold text-white transition-all shadow-lg hover:shadow-xl active:scale-95 disabled:opacity-50 ${
            type === '0' || type === '2' ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-rose-600 hover:bg-rose-500'
          }`}
        >
          {loading ? 'Processing...' : `Place ${['0', '2'].includes(type) ? 'Buy' : 'Sell'} Order`}
        </button>
      </form>
    </div>
  );
